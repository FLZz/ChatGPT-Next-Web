...
export const TEST_MASKS: BuiltinMask[] = [
  ...
  {
    avatar: "1f3e0",
    name: "产业分析（Industry analysis）",
    describe:
      "根据用户选定的产业主题的素材，进行产业分析和综述，使用户能够快速获取指定地区的产业情况简要分析结果。",
    context: [
      {
        id: "text-to-pic-0",
        role: "user",
  ...
  {
    avatar: "1f3e0",
    name: "产业分析2（Industry analysis 2）",
    describe:
      "根据用户选定的产业主题的素材，进行产业分析和综述，使用户能够快速获取指定地区的产业情况简要分析结果。",
    context: [
      {
        id: "text-to-pic-0",
        role: "user",
        content: `背景：我是一位产业分析员，我需要总结某个具体产业深度研究分析简报的构思目录大纲，以便更好地展开报告的撰写。
        角色：希望你扮演一个擅长产业分析的写作导师，能够提供清晰的指导和建议。
        任务：我将发送一些产业深度研究分析的基础素材给你，请你依据我提供的素材，提炼出产业主题（地名+产业类型），并帮我总结产业报告的构思大纲。
        要求：产业深度研究简报的大纲需要包含以下要素：行业的定义、主要发展趋势和变化、现状情况（包含技术发展、政策环境、市场需求、空间集聚状态）、地方内相关的产业规划有哪些。请以markdown格式提供具体和有逻辑性的两级大纲目录，以便更好地展开写作，大纲只能分为6个部分。你理解任务后只需要回复明白，等待我给你发送基础素材。在接下来的对话中，你只需要把结果直接输出，不需要其他多余的解释。`,
        date: "",
      },
      {
        id: "text-to-pic-1",
    ...
    autoTall: [
      `【任务设定】
        背景：你正在为一份专业的产业研究报告提供内容，这份报告将被城市规划专业人士和相关行业的决策者阅读。
        角色：你是一个资深的产业分析专家，擅长写专业、有深度且逻辑清晰的产业研究报告。
        【报告大纲】
        {
          $$$
        }
        【任务要求】
        1、文章输出：文章将按照大纲分成多个回答输出，现在请先根据上面的大纲，按章节结构输出前三个部分的内容，请你根据以下大纲，编写一篇3000字以上的深度研究报告。内容应详尽、专业、与素材高度相关，且以量化、客观数据支撑的描述为主。
        2、语言风格：请确保报告专业严谨，使用精确的数据和清晰的逻辑支持分析和结论。语言应保持正式和专业，避免使用口语化的表达。在叙述某个事情时需要有适当的扩展，思路逻辑不要太跳跃。
        3、段落风格：每个段落都应有明确的中心，表述清晰。段落之间的逻辑关系应明确，容易理解。通过段落的安排，确保重要信息或重要事件得到突出，避免相同信息的前后重复。
        4、深度要求：每个部分都需要具体深入的叙述，比如行业定义需要描述包含的子行业或关键技术、定位等信息的深入分析，切忌泛泛而谈。`,
      `【任务设定】
      背景：你正在为一份专业的产业研究报告提供内容，这份报告将被城市规划专业人士和相关行业的决策者阅读。
      角色：你是一个资深的产业分析专家，擅长写专业、有深度且逻辑清晰的产业研究报告。
      【报告大纲】
      ...
      【任务要求】
      5、文章输出：文章将按照大纲分成多个回答输出，现在请先根据上面的大纲，按章节结构输出后三个部分的内容，请你根据以下大纲，编写一篇3000字以上的深度研究报告。内容应详尽、专业、与素材高度相关，且以量化、客观数据支撑的描述为主。
      6、语言风格：请确保报告专业严谨，使用精确的数据和清晰的逻辑支持分析和结论。语言应保持正式和专业，避免使用口语化的表达。在叙述某个事情时需要有适当的扩展，思路逻辑不要太跳跃。
      7、段落风格：每个段落都应有明确的中心，表述清晰。段落之间的逻辑关系应明确，容易理解。通过段落的安排，确保重要信息或重要事件得到突出，避免相同信息的前后重复。
      8、深度要求：每个部分都需要具体深入的叙述，比如行业定义需要描述包含的子行业或关键技术、定位等信息的深入分析，切忌泛泛而谈。`,
    ],
  },
];
