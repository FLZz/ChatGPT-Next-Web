...
import { createDefaultMask, createEmptyMask, Mask } from "./mask";
import {
  ...
  (set, _get) => {
    ...
    const methods = {
      ...
      autoOutPush(mes: string, count: number = 0) {
        ...
        //     任务：请你按照【任务要求】，以提供的基础素材为依据，编写一篇3000字以上的深入分析具体产业的报告，要遵循叙述模型，保持报告章节间的连贯性和专业性。
        //     【任务要求】
        //     <报告大纲>
        //     {$$$}
        //     <创作流程>
        //     1、文章输出：文章将按照大纲分成多个回答输出，现在请先根据上面的大纲，按章节结构输出前三个部分的内容，内容尽量详细，与素材高度相关。
        //     <语言风格>
        ...
        //     {$$$}
        //     <创作流程>
        //     1、文章输出：文章将按照大纲分成多个回答输出，现在请先根据上面的大纲，按章节结构输出后三个部分的内容，内容尽量详细，与素材高度相关。
        //     <语言风格>
        //     1、专业严谨：确保报告的结论和论点逻辑清晰，便于读者理解。
        //     2、客观准确：使用精确的数据和清晰的逻辑支持分析和结论。
        //     3、正式客观：语言保持正式和专业，避免使用口语化的表达。
